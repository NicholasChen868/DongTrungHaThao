...
CREATE TABLE IF NOT EXISTS login_attempts (
    ...
    attempt_key TEXT NOT NULL,         -- e.g. 'member:0901234567' or 'admin'
...
CREATE OR REPLACE FUNCTION record_failed_attempt(p_key TEXT)
RETURNS VOID AS $$
BEGIN
    INSERT INTO login_attempts (attempt_key) VALUES (p_key);
...
CREATE OR REPLACE FUNCTION validate_admin(p_admin_hash TEXT)
RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
    ...
    IF is_rate_limited('admin', 3, 300) THEN
        RAISE EXCEPTION 'Quá nhiều lần thử đăng nhập admin. Đợi 5 phút.';
    END IF;
...
DECLARE
    ...
    rate_key TEXT;
BEGIN
    rate_key := 'ctv_reg:' || p_phone;

    -- Rate limit: 3 registrations per 30 minutes per phone
    IF is_rate_limited(rate_key, 3, 1800) THEN
        RAISE EXCEPTION 'Quá nhiều lần đăng ký. Vui lòng đợi 30 phút.';
    END IF;
    ...
    IF p_password_hash IS NULL OR p_password_hash !~ '^[a-f0-9]{64}$' THEN
