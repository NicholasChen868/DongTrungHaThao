...
CREATE POLICY "orders_anon_insert" ON orders FOR
...
CREATE POLICY "members_anon_insert" ON members FOR
INSERT TO anon WITH CHECK (
        name IS NOT NULL
        AND phone IS NOT NULL
...
CREATE POLICY "withdrawals_anon_insert" ON withdrawals FOR
INSERT TO anon WITH CHECK (
        status = 'pending'
...
DECLARE result JSON;
BEGIN -- Validate phone format (10 digits starting with 0)
...
-- =============================================
CREATE OR REPLACE FUNCTION admin_list_withdrawals(p_admin_hash TEXT) RETURNS JSON AS $$
...
BEGIN IF p_admin_hash != admin_hash THEN RAISE EXCEPTION 'Unauthorized';
END IF;
IF p_status NOT IN ('approved', 'rejected', 'paid') THEN RAISE EXCEPTION 'Invalid status';
END IF;
UPDATE withdrawals
SET status = p_status,
    updated_at = now()
...
CREATE OR REPLACE FUNCTION admin_get_overview(p_admin_hash TEXT) RETURNS JSON AS $$
DECLARE admin_hash CONSTANT TEXT := '285a242c372134fdfbea0c4c9b6a102c4b10134f1c5f6b4ad7dc016cc4f05889';
result JSON;
order_count INTEGER;
revenue BIGINT;
...
RETURN json_build_object(
    ...
    contact_count,
    'recent_orders',
    COALESCE(recent_orders, '[]'::json),
    'recent_ctv',
    COALESCE(recent_ctv, '[]'::json)
);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
-- admin_list_orders: Fix SELECT *
CREATE OR REPLACE FUNCTION admin_list_orders(p_admin_hash TEXT) RETURNS JSON AS $$
DECLARE admin_hash CONSTANT TEXT := '285a242c372134fdfbea0c4c9b6a102c4b10134f1c5f6b4ad7dc016cc4f05889';
result JSON;
BEGIN IF p_admin_hash != admin_hash THEN RAISE EXCEPTION 'Unauthorized';
END IF;
...
CREATE OR REPLACE FUNCTION admin_list_ctv(p_admin_hash TEXT) RETURNS JSON AS $$
DECLARE admin_hash CONSTANT TEXT := '285a242c372134fdfbea0c4c9b6a102c4b10134f1c5f6b4ad7dc016cc4f05889';
result JSON;
BEGIN IF p_admin_hash != admin_hash THEN RAISE EXCEPTION 'Unauthorized';
END IF;
