...
CREATE TABLE IF NOT EXISTS ctv_notifications (
    ...
    ctv_id UUID REFERENCES ctv_accounts(id),
    type TEXT NOT NULL,
    -- 'new_order', 'commission', 'withdrawal_approved', 'post_reward'
    title TEXT NOT NULL,
    message TEXT,
...
-- =============================================
CREATE OR REPLACE FUNCTION get_ctv_notifications(p_ref_code TEXT) RETURNS JSON AS $$
DECLARE v_ctv_id UUID;
result JSON;
unread_count INTEGER;
rate_key TEXT;
BEGIN -- Rate limit: 15 calls per 5 minutes
rate_key := 'notif:' || p_ref_code;
IF is_rate_limited(rate_key, 15, 300) THEN RAISE EXCEPTION 'Quá nhiều yêu cầu. Vui lòng đợi.';
END IF;
...
IF v_ctv_id IS NULL THEN RETURN json_build_object('ok', false, 'error', 'CTV not found');
END IF;
-- Count unread
...
-- =============================================
CREATE OR REPLACE FUNCTION notify_ctv_new_order() RETURNS TRIGGER AS $$
DECLARE v_ctv_id UUID;
v_amount TEXT;
BEGIN -- Chỉ xử lý khi có ctv_code
IF NEW.ctv_code IS NULL
OR NEW.ctv_code = '' THEN RETURN NEW;
END IF;
-- Tìm CTV
SELECT id INTO v_ctv_id
FROM ctv_accounts
WHERE referral_code = NEW.ctv_code;
IF v_ctv_id IS NULL THEN RETURN NEW;
END IF;
-- Format tiền
...
AFTER
INSERT ON orders FOR EACH ROW EXECUTE FUNCTION notify_ctv_new_order();